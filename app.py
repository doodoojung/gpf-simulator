...
# 2.1.1 - 2.1.2 ข้อมูลตั้งต้นและรายได้
gpf_current_balance = st.sidebar.number_input("💰 ยอดเงิน กบข. ปัจจุบัน (บาท)", min_value=0.0, value=284827.0, step=1000.0)
current_salary = st.sidebar.number_input("💵 เงินเดือนปัจจุบัน (บาท)", min_value=0.0, value=30870.0, step=1000.0)
...
st.sidebar.markdown("---")
vayupak_amount = st.sidebar.number_input("🏛️ จำนวนเงิน แผนกองทุนรวมวายุภักษ์ (บาท)\n(หากไม่มีให้ระบุ 0)", min_value=0.0, value=0.0, step=1000.0)
...
alloc_th_eq = st.sidebar.number_input("แผนหุ้นไทย (%)", min_value=0, max_value=100, value=20, step=5)
alloc_inter_eq = st.sidebar.number_input("แผนหุ้นต่างประเทศ (%)", min_value=0, max_value=100, value=20, step=5)
alloc_prop = st.sidebar.number_input("แผนกองทุนอสังหาริมทรัพย์ไทย (%)", min_value=0, max_value=100, value=0, step=5)
alloc_gold = st.sidebar.number_input(f"แผนทองคำ (%) [สูงสุด {gold_max_limit}%]", min_value=0, max_value=gold_max_limit, value=min(25, gold_max_limit), step=5)
alloc_th_fi = st.sidebar.number_input("แผนตราสารหนี้ (%)", min_value=0, max_value=100, value=25, step=5)
alloc_inter_fi = st.sidebar.number_input("แผนตราสารหนี้ต่างประเทศ (%)", min_value=0, max_value=100, value=10, step=5)
